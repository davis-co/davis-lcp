...
(function ($) {
  $.fn.nicelabel = function (options) {
    ...
    return this.each(function () {
      ...
      // استخراج رنگ از کلاس‌های input
      var colorClass = "";
      var inputClasses = obj.attr("class").split(" ");
      for (var i = 0; i < inputClasses.length; i++) {
        if (inputClasses[i].indexOf("--") > -1) {
          colorClass = inputClasses[i];
          break;
        }
      }
      ...
      if (obj.hasClass("circle-nicelabel")) {
        obj.after(create(input_id, "circle", position_class, colorClass));
      } else if (obj.hasClass("rect-nicelabel")) {
        obj.after(create(input_id, "rect", position_class, colorClass));
      } else if (obj.hasClass("text-nicelabel")) {
        obj.after(
          create(
            input_id,
            "text",
            ...
            checked_text,
            unchecked_text,
            settings.checked_ico,
            settings.unchecked_ico,
            colorClass
          )
        );
      }
  ...
  var create = function (
    ...
    // اضافه کردن کلاس رنگ به position_class
    var finalPositionClass = position_class;
    if (colorClass && colorClass !== "") {
      finalPositionClass += " " + colorClass;
    }
    ...
    if (label_theme == "circle") {
      ...
      if (uselabel == true) {
        label_html =
          ...
          input_id +
          '">' +
          '<span class="nicelabel-unchecked-image" style="background-image: url( ' +
          unchecked_ico +
          ' );"></span>' +
          '<span class="nicelabel-unchecked">' +
          unchecked_text +
          "</span>" +
          '<span class="nicelabel-checked-image" style="background-image: url( ' +
          checked_ico +
          ' );"></span>' +
          '<span class="nicelabel-checked">' +
          checked_text +
          "</span>" +
          "</label>";
      } else {
